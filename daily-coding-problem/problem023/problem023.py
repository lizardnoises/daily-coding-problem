...
simple though, because the edges are unweighted, so a breadth-first search will
do the job.
"""
...
def min_steps(b, s, e):
    """
    Returns the minimum number of steps needed to traverse the board b from
    the starting coordinate s to then ending coordinate e. Returns -1 if no
    such path exists.
    """
    visited = set()
    def valid(coord):
